...
def unet():
    ...
    d1 = down(input_net, 1)
    max1 = MaxPooling2D((2,2), (2,2), padding='same')(d1)

    d2 = down(max1, 2)
    max2 = MaxPooling2D((2,2), (2,2), padding='same')(d2)

    d3 = down(max2, 3)
    max3 = MaxPooling2D((2,2), (2,2), padding='same')(d3)

    d4 = down(max3, 4)
    max4 = MaxPooling2D((2,2), (2,2), padding='same')(d4)

    d5 = down(max4, 5)
    con1 = Conv2DTranspose(512, 3, 2, padding='same')(d5)

    u1 = up(con1, d4, 1)
    con2 = Conv2DTranspose(256, 3, 2, padding=pad)(u1)

    u2 = up(con2, d3, 2)
    con3 = Conv2DTranspose(128, 3, 2, padding='same')(u2)

    u3 = up(con3, d2, 3)
    con4 = Conv2DTranspose(64, 3, 2, padding='same')(u3)
